...
async function loadData() {
  const files = ['config','content','services','portfolio','testimonials','team','faqs'];
  await Promise.all(files.map(async name => {
    try {
      ...
      DATA[name] = ['services','portfolio','testimonials','team','faqs'].includes(name) ? [] : {};
    }
  }));
}
...
function renderNavbar() {
  ...
  mobile.querySelectorAll('.nav-link').forEach(l => l.addEventListener('click', () => {
...
function renderAbout() {
  ...
  const nums = [
    { value: cfg.experience, label: 'Years of Excellence', color: '#00d4ff' },
    { value: cfg.projects,   label: 'Projects Delivered',  color: '#ffd700' },
    { value: cfg.clients,    label: 'Happy Clients',       color: '#7c3aed' },
    { value: cfg.satisfaction,label:'Client Retention',    color: '#00ff88' }
  ];
    ...
    <div class="container">
      <div class="about-grid">
            ...
            <div class="about-number-grid">
              ${nums.map(n => `
                <div class="about-num-card">
                  <div class="about-num" style="color:${n.color}">${n.value}</div>
                  <div class="about-num-label">${n.label}</div>
                </div>`).join('')}
            </div>
        ...
        <div class="about-content reveal-right">
          ...
          <div class="mt-4">
            <a href="#contact" class="btn-primary">${ic('arrow-right',16)} Start a Project</a>
          </div>
        </div>
      </div>
    </div>`;
...
function renderTestimonials() {
  ...
  const goTo = (i) => {
    ...
    dots.querySelectorAll('.slider-dot').forEach((d,j) => d.classList.toggle('active', j === slideIndex));
  };

  document.getElementById('slider-prev').addEventListener('click', () => { clearInterval(slideTimer); goTo(slideIndex - 1); startAutoplay(); });
  document.getElementById('slider-next').addEventListener('click', () => { clearInterval(slideTimer); goTo(slideIndex + 1); startAutoplay(); });
  dots.querySelectorAll('.slider-dot').forEach(d => d.addEventListener('click', () => { clearInterval(slideTimer); goTo(+d.dataset.i); startAutoplay(); }));

  const startAutoplay = () => { slideTimer = setInterval(() => goTo(slideIndex + 1), 5500); };
  startAutoplay();
}
        ...
        <div class="contact-info-card reveal-left">
          ...
          <div class="contact-social">
            ...
            <div class="contact-social-links">
              ${cfg.social.map(s => `
                <a href="${s.url}" class="social-icon-link" title="${s.name}" target="_blank" rel="noopener">${ic(s.icon,16)}</a>`).join('')}
            </div>
          </div>
...
function handleFormSubmit(e) {
  ...
  if (!message) { showFormError('Please enter your message.'); return; }
    ...
    <div class="container">
      <div class="footer-grid">
        ...
        <div>
          ...
          ${footer.quickLinks.map(l => `<a href="${l.href}" class="footer-link">${l.label}</a>`).join('')}
        </div>
        <div>
          <div class="footer-col-title">Our Services</div>
          ${footer.services.map(l => `<a href="${l.href}" class="footer-link">${l.label}</a>`).join('')}
        </div>
        <div>
          <div class="footer-col-title">Get In Touch</div>
          <a href="mailto:${site.email}" class="footer-link footer-contact-link"><span>${site.email}</span></a>
          <a href="tel:${site.phone}" class="footer-link footer-contact-link"><span>${site.phone}</span></a>
          <p class="footer-link footer-contact-link"><span>${site.address}</span></p>
          <p class="footer-link footer-contact-link"><span>${site.timing}</span></p>
        </div>
      </div>
...
function initCursor() {
  ...
  document.querySelectorAll('a, button, .service-card, .portfolio-card, .filter-btn')
    .forEach(el => {
      el.addEventListener('mouseenter', () => { ring.style.transform = 'translate(-50%,-50%) scale(1.5)'; ring.style.opacity = '0.7'; });
